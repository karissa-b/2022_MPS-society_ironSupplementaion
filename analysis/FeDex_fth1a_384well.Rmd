...
Here, we are assessing the level of *fth1a* transcripts in the brains of 6 month old zebrafish. These zebrafish are either heterozygous (unaffected) or homozygous (MPS-IIIB) for the A603fs mutation in *naglu*. They were treated with either vehicle (0.85% saline), 0.5 μg iron-dextran or 1 μg of iron-dextran . Then, they were allowed to recover alone in a pair mate tank for either 7 or 14 days. Here, we will determine whether there are significant changes to the expression of *fth1a* due to *naglu* genotype and treatment. 

The *fth1a* transcript has an iron responsive element in the 5' untranslated region (3' UTR), and is predicted to decrease under high iron levels. 

Previously, Ewan tried to assess the expression of fth1a using qPCRs using 96 well plate (see [here](FeDex_fth1a.html)). However, between-plate effects may have been masking any effect of genotype amnd/or treatment. Therefore, he has repeated the experiment using 96-well plates. This means that all samples, replicates and controls can be performed on the same plate and the batch effect is eliminated. 

```{r}
# read in daa
data <- read_xlsx("data/2022-10-25_193133_fth1a_experiment_2_384well.xlsx", 
...
The following plot below shows the technical replicates of the fth1a gene. 2 technical replicates appear to be problematic. Ewan noted these in his lab notebook though 

```{r}
# plot technical replicates for tfr1a by plate. 
data %>% 
  dplyr::filter(fish_id %in% c("NTC", seq(1:100))) %>% # omit those dilution PCRs that ewan was playing with
  dplyr::filter(`Target Name` == "fth1a") %>% 
  unique %>% 
  mutate(fish_id = factor(fish_id, levels = c("NTC", seq(1:100)))) %>% # put in order
  ggplot(aes(x = fish_id, y = CT)) + 
  geom_point() +
...
Upon inspection of the two tech replicate plots, I will omit the following replicates:
"C1", "E13", "I1", "K13", "M1", "C4", "E4", "G4", "I4", "K4", "M4"

```{r}
data_clean <- data %>%
  dplyr::filter(
    !(`Well Position` %in% c("I12","M5"))
  )
  
```

# repeat the technical replicate plots
...
Expression of *fth1a* does not appear to change with iron treatment 
...
data_clean %>% 
  ...
  ggtitle("boxplots of fth1a", 
          subtitle = "expression of fth1a relative to beta actin") 
...
data_clean %>% 
  ...
  scale_colour_viridis_d(end = 0.5) +
    ggtitle("barplots showig mean expression of fth1a", 
          subtitle = "expression of fth1a relative to beta actin") 
```
...
Nevertheless, I checked the significnace of the effects and their interactions using type II Anova. The effect of `genotype * treatment * dpi` is not significant. 
