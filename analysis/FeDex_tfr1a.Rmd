...
The following plot below shows the technical replicates of the tfr1a gene. Again, there seem to be some dodgy ones, likely due to pipetting error or some contamination of the RNA. The technical replicate which appears to be the problematic one will be omitted. 
...
Repeating the technical replicate plots and now the spread of the technical replicates per fish look good. 
...
data_clean %>% 
  ...
  dplyr::filter(`Target Name` == "tfr1a") %>% 
  unique %>% 
  mutate(fish_id = factor(fish_id, levels = c("NTC", seq(1:100)))) %>% 
  ggplot(aes(x = fish_id, y = CT)) + 
  geom_point() +
  geom_boxplot(fill = NA) +  
  ggtitle("tfr1a technical replicates"), 
...
data_clean %>% 
  ...
  scale_colour_viridis_d(end = 0.5) +
    ggtitle("barplots showig mean expression of tfr1a", 
          subtitle = "expression of tfr1a relative to beta actin") 
```
